...
float Vol;
extern Encoder CM1Encoder, CM2Encoder, CM3Encoder, CM4Encoder;
extern u8 can_test;
/**************************************************************************
函数功能：OLED显示
入口参数：无
...
void oled_show(void)
{
  ...
  {		
		//=============显示电机A的状态=======================//	
	  if( can_test<0)		    OLED_ShowString(00,10,"-"),
		                      OLED_ShowNumber(15,10,-can_test,5,12);
		else                 	OLED_ShowString(0,10,"+"),
		                      OLED_ShowNumber(15,10, can_test,5,12); 
		
		if( CM1Encoder.filter_rate<0)		  OLED_ShowString(80,10,"-"),
		                      OLED_ShowNumber(95,10,-CM1Encoder.filter_rate,4,12);
		...
		if( CM2Encoder.filter_rate<0)		  OLED_ShowString(80,20,"-"),
		                      OLED_ShowNumber(95,20,-CM2Encoder.filter_rate,4,12);
		...
		if( CM3Encoder.filter_rate<0)		  OLED_ShowString(80,30,"-"),
		                      OLED_ShowNumber(95,30,-CM3Encoder.filter_rate,4,12);
		...
		if( CM4Encoder.filter_rate<0)		    OLED_ShowString(80,40,"-"),
		                      OLED_ShowNumber(95,40,-CM4Encoder.filter_rate,4,12);
...
{    
	  ...
	  app_2=Move_X*0.7f;  if(app_2<0)app_2=-app_2;			                   //对编码器数据就行数据处理便于图形化
		app_3=Move_Y*0.7f;  if(app_3<0)app_3=-app_3;
